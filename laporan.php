    ...
    <section class="content">
      <div class="row">
        <div class="col-xs-12">
          ...
          $no = 1;
          $total_semua = 0;
          $stid = oci_parse($koneksi, 'SELECT jadwal.*, matkul.*, transaksi.* FROM transaksi 
transaksi INNER JOIN jadwal jadwal ON transaksi.id_jadwal = jadwal.id_jadwal 
INNER JOIN matkul matkul ON transaksi.KD_MK = matkul.KD_MK ORDER BY transaksi.id_tran ASC');
          ...
          $total_semua += $total; 
            ...
            <tr>
              ...
              <td><?= rupiah($row['FAKULTAS']); ?></td> 
              <td><?= rupiah($total); ?></td> 
              <td class="td-actions">
              <a class='btn btn-success' href="edit_laporan.php?ID_TRAN=<?= $d["ID_TRAN"];?>"
              ><i class='fa fa-edit'></i></a>
              <a class='btn btn-danger' onclick="return confirm('Anda yakin ingin menghapus data tersebut?');" 
              href="hapus_laporan.php?ID_TRAN=<?= $d["ID_TRAN"];?>" 
